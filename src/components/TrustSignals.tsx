export function TrustSignals() {
  return (
    <section className="py-6 md:py-10 bg-black">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <p className="text-center text-xs md:text-sm text-gray-400 uppercase tracking-wider mb-4 md:mb-8">
          Certified &amp; Trusted
        </p>
        <div className="grid grid-cols-2 md:grid-cols-6 items-center justify-center gap-6 md:gap-10">
          <img
            src="/assets/1458f26437ca425f03f4b0d18ae3ec02eeae4c05.png"
            alt="NQA AS9100 Rev D Aerospace & Defense Certification - ISO 9001:2015"
            className="h-16 md:h-20 w-auto mx-auto"
          />
          <img
            src="/assets/cgp-logo.svg"
            alt="Controlled Goods Program - Government of Canada"
            className="h-14 md:h-16 w-auto mx-auto"
          />
          <img
            src="/assets/canada-made-logo.png"
            alt="Made in Canada"
            className="h-14 md:h-20 w-auto invert mx-auto"
          />
          <img
            src="/assets/ontario-made-logo.png"
            alt="Ontario Made"
            className="h-12 md:h-16 w-auto mx-auto"
          />
          <img
            src="/assets/dair-logo.png"
            alt="DAIR - Downsview Aerospace Innovation & Research"
            className="h-10 md:h-14 w-auto invert mx-auto"
          />
          <img
            src="/assets/ctma-logo.jpg"
            alt="CTMA - Canadian Tooling & Machining Association"
            className="h-10 md:h-12 w-auto mx-auto"
          />
        </div>
      </div>
    </section>
  );
}
